...
if (-not (Test-Path $governanceDir)) {
    New-Item -ItemType Directory -Path $governanceDir | Out-Null
    Write-Output "Created: docs/governance/"
}

$reportPath = Join-Path $governanceDir "audit-report.md"
...
if ((-not (Test-Path $reportPath)) -or $ForceScaffold) {
    ...
    Write-Output "Scaffolded: docs/governance/audit-report.md"
} else {
    Write-Output "Existing report kept: docs/governance/audit-report.md"
}
...
Write-Output "2. Paste machine evidence into docs/governance/audit-report.md"
...
Write-Output "7. Write publication-decision-record.md when phase=pre-public (G-20)"
Write-Output "8. If R-02 blocked with accepted risk, write docs/governance/accepted-risk-record.md"
